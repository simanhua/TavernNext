...
let installed = false;
const catalog = {
  sceneId, version: '2.1.0', packageUrl: 'builtin:destined-poem',
...
const installedScene = {
  ...
  slug: 'destined-poem', version: '2.1.0', archiveDigest: 'a'.repeat(64), installPath: 'scenes/test',
  installedAt: new Date().toISOString(), backingCharacterId: '018f2000-0000-7000-8000-000000000002',
  manifest: { id: sceneId, slug: 'destined-poem', version: '2.1.0', name: catalog.name, summary: catalog.summary, description: '', author: catalog.author, minimumTavernNextVersion: '1.0.0', sceneSdkVersion: 2, frontendEntry: 'frontend/app.js', frontendStyles: ['frontend/styles.css'], coverPath: 'content/cover.png', setupSchema: {}, stateSchema: {}, files: ['frontend/app.js', 'frontend/styles.css', 'content/cover.png'] },
  coverUrl: `/api/scenes/${sceneId}/assets/content/cover.png`,
...
const server = setupServer(
  ...
  http.get('/api/scenes/catalog', () => HttpResponse.json([{
    ...catalog,
    installed,
    ...(installed ? { coverUrl: installedScene.coverUrl } : {}),
  }])),
...
describe('SceneLibraryPage', () => {
  it('installs an official Scene and promotes it into the card library', async () => {
    ...
    const covers = screen.getAllByRole('img', { name: '命定之诗与黄昏之歌 封面' });
    expect(covers).toHaveLength(2);
    expect(covers.every((cover) => cover.getAttribute('src') === installedScene.coverUrl)).toBe(true);
  });
});
