    ...
    <main className="settings-center">
      <aside className="settings-sidebar">
        ...
        <nav aria-label="设置分区">
          ...
          <button className={tab === 'fallback' ? 'active' : ''} onClick={() => setTab('fallback')}>Chat 模板</button>
        </nav>
      </aside>
      <section className="settings-content">
        {tab === 'connection' ? <ConnectionPage /> : tab === 'personas' ? <PersonaManagerPage /> : <PresetManagerPage />}
      </section>
    </main>
  );
}
