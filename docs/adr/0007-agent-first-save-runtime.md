...
Prompt precedence is fixed: platform contracts, world rules, Character identity, Save Agent Configuration, current Scene State and history, then player input. The World Rules layer inlines a deterministic tiered subset of activated entries—always retaining `ignoreBudget` rules, then turn-relevant and priority-ranked entries—while deferred entries remain queryable through `world_query`. A Preset controls writing and sampling but cannot suppress higher layers, tools, or output contracts. Browser prompt hooks, candidate sealing, continuation, Text Completion, and prose `<UpdateVariable>` state extraction are removed.
